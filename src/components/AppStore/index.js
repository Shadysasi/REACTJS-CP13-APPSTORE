...
class AppStore extends Component {
  ...
  getFilterApps = searchApps => {
    const {activeTabId} = this.state
    const filteredApps = searchApps.filter(
      eachList => eachList.category === activeTabId,
    )
    return filteredApps
  }
  ...
  render() {
    ...
    const filteredApps = this.getFilterApps(searchResults)
